...
//  YourTime&Theirs
//
//  Created by Bharat Kumar on 11/6/20.
...
class NotificationController {
    ...
    func scheduleNotification(timer: TimerEvent) -> String? {
        
        if timer.timerUuid != nil { return nil }
        guard let dateTime = timer.timeLeft else { return nil }
        
        let center = UNUserNotificationCenter.current()
        
        let content = UNMutableNotificationContent()
        content.title = "Meet Up Now!"
        content.body = "Your Even is here  \(timer.name)\nGood News! It's done!"
    ...
    func cancelNotification(timer: TimerEvent) {
